    ...
    <div className="flex flex-col min-h-screen bg-white">
              ...
              {liked.map((prod) => (
                <Link href={`/products/${prod.id}`} key={prod.id} className="block group">
                  <div className="bg-white rounded-2xl shadow hover:shadow-xl hover:scale-[1.015] transition flex flex-col items-center p-5">
                    ...
                    <div className="font-bold text-lg mb-1 truncate w-full text-center">
                      {prod.title}
                    </div>
      ...
      <ConfirmDialog
        ...
        description={
          confirmProduct && (
            <span>
              Tu pourras toujours revenir liker la{" "}
              <span className="font-semibold">{confirmProduct.title}</span> plus tard.
            </span>
          )
        }
