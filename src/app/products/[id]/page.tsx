...
export default function ProductPage() {
    ...
    ;(async () => {
      ...
      if (currentUser) {
        const { data: profile } = await supabase
          ...
          .single()
        if (profile?.pointure) setUserShoeSize(profile.pointure)
      }
  ...
  async function handleVote() {
    ...
    toast.success(result.message ?? "Vote enregistré ✅")
    setUserVoted(true)
  ...
  async function handleSaveSize() {
    if (!user) return
    const { error } = await supabase.from("profiles").update({ pointure: userShoeSize }).eq("id", user.id)
    if (!error) {
